---
title: `Figures for the paper- Evaluation of uncertainties in the anthropogenic SO2 emissions in the USA from NASA’s OMI point source catalog`
...
Eia_Data_Mean <- Get_Consolidated_OMI_Data(source= "EIA", startrange = dmax, endrange = dmax, rangesequence = 10,minvalue = 0,years=EIA_years) %>% 
                 inner_join(EIA_Data_ISOL, by= c("PlantName","Year"))

Nei_Data_Mean <- bind_rows(Get_Consolidated_OMI_Data(source= "NEI", startrange = dmax, endrange = dmax, rangesequence = 10,minvalue = 0,years = NEI_years))         %>% 
                 inner_join(NEI_Data_ISOL, by= c("PlantName","Year"))


EGRIDS_Data_Mean <- bind_rows(Get_Consolidated_OMI_Data(source= "EGRIDS", startrange = dmax, endrange = dmax, rangesequence = 10,minvalue = 0,years = EGRIDS_years))         %>% 
...
g <- ggplot()+
  ...
  ylab("Error in kt/year")+
  ggtitle("Error for sources included in catalog for different radii")+
  facet_wrap(~source)
...
EGRIDS_Data_Mean %>% bind_rows(Eia_Data_Mean %>% filter(Year %notin% c(EGRIDS_years,NEI_years)),Nei_Data_Mean %>% filter(Year %notin% c(EGRIDS_years))) %>%  mutate(Valueplant=Value,source="OMI (Catalog)",count=1) %>% group_by(Range, Year,source) %>% mutate(Error_isol= (sum((Valueplant)))) %>%
                   #mutate(count=sum(reference_col)) %>% 
                   ungroup() %>% 
...
Eia_Data_Mean %>% mutate(source= "EIA") %>% rename(Valueplant= ValuefromEIAinKT) %>% bind_rows(Nei_Data_Mean %>% mutate(source="NEI (Comprehensive point sources)") %>% rename(Valueplant= ValuefromNEIinKT),EGRIDS_Data_Mean %>% mutate(source="EGRIDS (point source database for powerplants)")%>% rename(Valueplant= ValuefromEGRIDSinKT)) %>% select(-ValuefromEIA,-ValuefromNEI, -ValuefromEGRIDS) %>%   na.omit() %>% 
                   ...
                   distinct() %>% mutate(Status="1. Included Emissions")->Isol_stats_Detected
...
g<-ggplot(data=bind_rows(Isol_stats_Detected) %>% filter(source != "EIA") %>% mutate(inventory=source)  , aes(x=Year,y=Error_isol, color= source)) +
  scale_color_manual(values= c("blue", "red", "black"))+
  geom_line()+
  geom_line(data=bind_rows(OMI_Data %>% select(-source))  , aes(x=Year,y=Error_isol, color= "OMI data (Catalog)"), linetype = "solid",show.legend = FALSE,size=1.3)+
  geom_point(show.legend = FALSE)+
  geom_point(data=bind_rows(OMI_Data %>% select(-source))  , aes(x=Year,y=Error_isol, color= "OMI data (Catalog)"),show.legend = FALSE)+
  scale_linetype_manual(values=c("solid","dotted","solid"))+
  ggtitle(paste0("Total SO2 emissions for included sources in catalog and inventories")) +
...
#Figure 3:Errors across years for included sources (Percent and Total)
```{r,fig.height=6, fig.width=10}
...
Eia_Data_Mean %>%  na.omit() %>% 
                   ...
                   distinct() %>%   
                   mutate(Type = "Total error (in kt/year)",source="1. EIA",Year= as.integer(Year))->Isol_stats_abs
...
Nei_Data_Mean %>%  na.omit() %>% 
                   ...
                   select(Range,Year,Error_isol,-reference_col,count) %>%
                   distinct() %>%   
                   mutate(Type = "Total error (in kt/year)",Year= as.integer(Year),source="3. NEI")->Isol_stats_NEI_abs
...
EGRIDS_Data_Mean %>%  na.omit() %>% 
                   ...
                   select(Range,Year,Error_isol,-reference_col,count) %>%
                   distinct() %>%   
                   mutate(Type = "Total error (in kt/year)",source="2. EGRIDS",Year= as.integer(Year))->Isol_stats_EGRIDS_abs
...
Eia_Data_Mean %>%  na.omit() %>% 
                   ...
                   select(Range,Year,Error_isol,-reference_col,count) %>%
                   distinct() %>% 
                   mutate(Type ="Percent error per year",source="1. EIA",Year= as.integer(Year))->Isol_stats_percent
...
Nei_Data_Mean %>%  na.omit() %>% 
                   ...
                   select(Range,Year,Error_isol,-reference_col,count) %>%
                   distinct() %>%   
                   mutate(Type ="Percent error per year",Year= as.integer(Year),source="3. NEI")->Isol_stats_NEI_percent
...
EGRIDS_Data_Mean %>%  na.omit() %>% 
                   ...
                   select(Range,Year,Error_isol,-reference_col,count) %>%
                   distinct() %>%   
                   mutate(Type ="Percent error per year",source="2. EGRIDS",Year= as.integer(Year))->Isol_stats_EGRIDS_percent

EGRIDS_Data_Mean %>% filter((Error/ValuefromEGRIDSinKT)<2) %>% filter(Status=="Detected") %>%  na.omit() %>% mutate(Type= "Percent error per year",value=((Error)/ValuefromEGRIDSinKT)*100,Year= as.integer(Year))-> Jitter_data_percent

EGRIDS_Data_Mean %>% filter((Error/ValuefromEGRIDSinKT)<2) %>% filter(Status=="Detected") %>%  na.omit() %>% mutate(Type= "Total error (in kt/year)",value=((Error)),Year= as.integer(Year))-> Jitter_data_abs

Nei_Data_Mean %>%  filter((Error/ValuefromNEIinKT)<2) %>% filter(Status=="Detected") %>%  na.omit() %>% mutate(Type= "Percent error per year",value=((Error)/ValuefromNEIinKT)*100,Year= as.integer(Year))-> Jitter_data_print
...
g<-ggplot(data=Isol_stats_abs %>% bind_rows(Isol_stats_percent,Isol_stats_EGRIDS_abs,Isol_stats_EGRIDS_percent), aes(x=Year,y=Error_isol)) +
  ...
  ggtitle(paste0("Errors across years for included sources (Percent and Total)")) +
  facet_wrap(~Type, scales="free_y",labeller = as_labeller(c(`Percent error per year`="Percent error per year (OMI- inventory)",
                                                             `Total error (in kt/year)`="Error in kt/year (OMI- inventory)")),strip.position = "left") +
...
g+scheme_basic+theme(strip.background = element_blank(), strip.placement = "outside")
...
g <- ggplot(data= tmp_EGRIDS %>% bind_rows(tmp_NEI), aes(x=bin, y=sd_value, color=inventory))+
   ...
   ylab("Percent error")+facet_wrap(~inventory)+xlab("Size of emissions in kt/year")+
   labs(subtitle = "Observations with emissions under 10 kt are not included. Numbers represent count of included sources (n) in all years")
...
EGRIDS_Data_Mean %>%
  #First, calculate the error as OMI-EGRIDS
  mutate(Error=abs(Value-ValuefromEGRIDSinKT)) %>%
  #Now filter out detected sources
  filter(Status=="Detected") %>%
  #Add selection criteria. Add counts and calculate average emissions
  mutate(Obs=1) %>%
  ...
  mutate(obs =1) %>% 
  group_by(bin) %>% 
  mutate(sd_value = sd(Error), mean_value = mean(Error),count=as.integer(sum(obs))) %>% 
  ungroup() %>% 
  select(bin, sd_value, mean_value, count) %>% 
  distinct() %>% 
  filter(bin != "0-10") %>% 
...
Nei_Data_Mean %>%
  #First, calculate the error as OMI-EGRIDS
  mutate(Error=abs(Value-ValuefromNEIinKT)) %>%
  #Now filter out detected sources
  filter(Status=="Detected") %>%
  #Add selection criteria. Add counts and calculate average emissions
  mutate(Obs=1) %>%
...
g <- ggplot(data= tmp_EGRIDS %>% bind_rows(tmp_NEI), aes(x=bin, y=sd_value, color=inventory))+
     ...
     geom_point(data=tmp_EGRIDS%>% bind_rows(tmp_NEI), aes(x=bin, y= mean_value, shape="mean",color= inventory),size=3)+
     ggtitle("Error statistics vs emissions (in bins) from EGRIDS and NEI")+
   ylab("Error in kt")+facet_wrap(~inventory)+xlab("Size of emissions in kt")+
   labs(subtitle = "Observations with emissions under 10 kt are not included. Numbers represent count of included sources (n) in all years")

g+scheme_basic
ggsave( paste0('4b.OutputsError statistics vs emissions (in bins) from EGRIDS and NEI','.png'),width = 12, height = 6 )
...
#Figure 6:Size of bias (mean percent error) and Direction of bias (over vs underestimation) for included sources"
```{r,fig.height=6,fig.width=10}
...
tmp_NEI %>% left_join((tmp_NEI_direction), by= c("PlantName","source")) %>%
  na.omit() %>% mutate(source = "NEI (Comprehensive point sources)") %>% dplyr::select(PlantName,Error_Term,Error,Average_Emissions_KT,source) %>% distinct()->plot_data

g <- ggplot(data=plot_data %>% filter(source == "NEI (Comprehensive point sources)"), aes(x=Error_Term, y=Error, color=source))+
     ...
     ylab("Mean error over time in kt (Size of bias over time)")+
     ggtitle("Size of bias (mean error in kt) and Direction of bias (over vs underestimation) for included sources")+
...
x <- seq(min(plot_data$Error), max(plot_data$Error), length.out=56)


m <- mean(plot_data$Error)
s <- sd(plot_data$Error)
location <- log(m^2 / sqrt(s^2 + m^2))
shape <- sqrt(log(1 + (s^2 / m^2)))

draws3 <- dnorm(seq(-50, 50, length.out=43), 0, s)
draw_d <- as.data.frame(draws3)  %>% 
          mutate(Error = seq(-50, 50, length.out=43)) %>% 
 
           rename(density = draws3 )
d <- density(na.omit(plot_data$Error),n=1e4)
...
FWHM <- as.integer(x2-x1)

g <- ggplot()+
     geom_histogram(data= plot_data, aes(x=Error, y = ..density..),fill = "white",color="black", bins = 20)+
  geom_density(data= plot_data, aes(x=Error, y = ..density..,label="Smoothed density based on error distribution"),color="red", size = 1.2)+
  geom_line(data= draw_d, aes(x=Error, y=density),color="blue", linetype = "dashed", size = 1.2)+
  geom_text(data=plot_data, aes(x= max(90)*0.8,y=max(0.01),label= paste0("FWHM - ",FWHM)))+
  geom_text(data=plot_data, aes(x= max(90)*0.77,y=max(0.0090),label= paste0("n - ",length(plot_data$PlantName))))+
  #scale_linetype_manual(name = 'the fill', guide = 'legend',labels = c('m1')) +
  xlab("Mean error in kt")+
  xlim(-75,90)+
  ggtitle("Density plot of error for all included sources in the NEI inventory")+
  labs(subtitle = "Bars are the actual density of the error. The red line is a smoothed density curve.FWHM is the Full Width at Half Maximum calculated for the distribution.")
g+scheme_basic
...
Eia_Data_Mean %>% mutate(source= "EIA (CEMS powerplants)") %>% rename(Valueplant= ValuefromEIAinKT) %>% bind_rows(Nei_Data_Mean %>% mutate(source="NEI (Comprehensive point sources)") %>% rename(Valueplant= ValuefromNEIinKT),EGRIDS_Data_Mean %>% mutate(source="EGRIDS (powerplant point sources)")%>% rename(Valueplant= ValuefromEGRIDSinKT)) %>% select(-ValuefromEIA,-ValuefromNEI, -ValuefromEGRIDS) %>%   na.omit() %>% 
                   ...
                   ungroup() %>% 
                   select(Range,Year,Error_isol,-reference_col,count,source) %>%
                   distinct() %>% mutate(Status="1. Included in catalog")->Isol_stats_Detected
...
Eia_Data_Mean %>% mutate(source= "EIA (CEMS powerplants)") %>% rename(Valueplant= ValuefromEIAinKT) %>% bind_rows(Nei_Data_Mean %>% mutate(source="NEI (Comprehensive point sources)") %>% rename(Valueplant= ValuefromNEIinKT),EGRIDS_Data_Mean %>% mutate(source="EGRIDS (powerplant point sources)")%>% rename(Valueplant= ValuefromEGRIDSinKT)) %>% select(-ValuefromEIA,-ValuefromNEI, -ValuefromEGRIDS) %>%   na.omit() %>% 
                   ...
                   mutate(count=sum(reference_col)) %>% 
                   ungroup() %>% 
                   select(Range,Year,Error_isol,-reference_col,count,source) %>% distinct() %>% mutate(Status="2. Not included in catalog")->Isol_stats_Undetected
...
```{r,fig.width=10,fig.height=6}
Nei_Data_Mean %>% 
  mutate(Status = if_else(Status !="Detected", "Not included in catalog", "Included in catalog")) %>% 
...
g <- ggplot(data =t %>% filter(bin != "0-5"), aes(x=bin, fill=Status))+
     ...
     ggtitle("Distribution of emissions for sources by inlusion status* for all years for the NEI")+
...
##  SI Figure 3- Distribution of emissions for sources by inclusion status* for pre and post 2010 period.
```{r}
Nei_Data_Mean %>% 
  mutate(Status = if_else(Status !="Detected", "Not included in catalog", "Included in catalog")) %>%
  mutate(Category = if_else(Year <= 2010, "Pre 2010", "Post 2010")) %>% 
  mutate(bin = if_else(ValuefromNEIinKT < 5 , "0-5",
                       if_else(ValuefromNEIinKT < 10 , "5-10",
                       if_else(ValuefromNEIinKT < 20,"10-20",
                       if_else(ValuefromNEIinKT < 25, "20-25",
...
## SI Figure 1- Density plot of percent error for all included sources in the EGRIDS inventory (Based on first 3 years of data)
```{r}
EGRIDS_Data_Mean %>%
  mutate(Error=Value-ValuefromEGRIDSinKT) %>%
  #Now filter out detected sources
  filter(Status=="Detected",
         Year %in% c(2005,2007,2009)) %>%
  #Add selection criteria. Add counts and calculate average emissions
  mutate(Obs=1) %>%
  group_by(PlantName) %>% 
  mutate(obs_count=sum(Obs)) %>% 
  ungroup() %>% 
  group_by(PlantName) %>%
  mutate(avg_em= mean(ValuefromEGRIDSinKT),Error=mean(Error)) %>%
  ungroup() %>%
  #Now, let's filter out on the basis of the criteria. Average emissions should be over 20 kt & you should
  #have at least 3 observations
  #filter(avg_em>20) %>%
  filter(obs_count>=3) %>%
  mutate(Error_percent= (Error/avg_em)*100, source="NEI",Valueplant = ValuefromEGRIDSinKT) %>% 
  #Now let's calculate the data for the plot
  select(PlantName,Error,source) %>%
  distinct()->tmp_EGRIDS


tmp_EGRIDS %>%  distinct() ->plot_data
x <- seq(min(plot_data$Error), max(plot_data$Error), length.out=56)
...
draws3 <- dnorm(seq(-50, 50, length.out=56), 0,60 )
draw_d <- as.data.frame(draws3)  %>% 
          mutate(Error = seq(-50, 50, length.out=56)) %>% 
          rename(density = draws3 )
...
unique_plants <- unique(plot_data$PlantName) 
g <- ggplot()+
     geom_histogram(data= plot_data, aes(x=Error, y = ..density..),fill = "white",color="black", bins = 20)+
  geom_density(data= plot_data, aes(x=Error, y = ..density..),color="red", size = 1.2)+
  #geom_line(data= draw_d, aes(x=Error, y=density),color="blue", linetype = "dashed", size = 1.2)+
  geom_text(data=plot_data, aes(x= max(60)*0.8,y=max(0.01),label= paste0("FWHM - ",FWHM)))+
  geom_text(data=plot_data, aes(x= max(60)*0.75,y=max(0.01*0.85),label= paste0("n - ",length(plot_data$PlantName))))+
  xlab("Mean error in kt")+
  xlim(-50,60)+
  ggtitle("Density plot of percent error for all detected sources in the EGRIDS inventory (Based on first 3 years of data)")+
  labs(subtitle = "Bars are the actual density of the error. The red line is a smoothed density curve")
  #ylim(0,0.0125)
g+scheme_basic

ggsave( paste0('Outputs/Density plot of percent error for all detected sources in the EGRIDS inventory ( First 3 years)','.png'),width = 10, height = 6 )
```

## SI Figure 2- Density plot of percent error for all unincluded sources in the EGRIDS inventory (Based on first 6 years of data)
```{r}
EGRIDS_Data_Mean %>%
  mutate(Error=Value-ValuefromEGRIDSinKT) %>%
  #Now filter out detected sources
  filter(Status=="Detected",
         !Year %in% c(max(EGRIDS_years))) %>%
  #Add selection criteria. Add counts and calculate average emissions
  mutate(Obs=3) %>%
  group_by(PlantName) %>% 
  mutate(obs_count=sum(Obs)) %>% 
...
tmp_EGRIDS %>%  distinct() %>% filter(PlantName %in% c(unique_plants))->plot_data
x <- seq(min(plot_data$Error), max(plot_data$Error), length.out=56)
...
draws3 <- dnorm(seq(-150, 150, length.out=56), 0,60 )
draw_d <- as.data.frame(draws3)  %>% 
          mutate(Error = seq(-150, 150, length.out=56)) %>% 
          rename(density = draws3 )
...
g <- ggplot()+
  ...
  geom_density(data= plot_data, aes(x=Error, y = ..density..),color="red", size = 1.2)+
  #geom_line(data= draw_d, aes(x=Error, y=density),color="blue", linetype = "dashed", size = 1.2)+
  geom_text(data=plot_data, aes(x= max(60)*0.8,y=max(0.02),label= paste0("FWHM - ",FWHM)))+
  geom_text(data=plot_data, aes(x= max(60)*0.76,y=max(0.02*0.85),label= paste0("n - ",length(plot_data$PlantName))))+
  xlab("Mean error in kt")+
  xlim(-50,60)+
  ggtitle("Density plot of percent error for all detected sources in the EGRIDS inventory (Based on 6 years of data)")+
  labs(subtitle = "Bars are the actual density of the error. The red line is a smoothed density curve")
  #ylim(0,0.0125)
g+scheme_basic

ggsave( paste0('Outputs/Density plot of percent error for all detected sources in the EGRIDS inventory (6 years)','.png'),width = 10, height = 6 )
```
## SI Figure 4 – QQ plot showing actual mean error in kt vs normalized mean error.
```{r,fig.width=10,fig.height=6}

g <- ggplot(data=Nei_Data_Mean %>% filter(Status =="Detected") %>% 
              group_by(PlantName) %>% 
              mutate(Error= mean(Error)) %>% 
              ungroup() %>% 
              select(PlantName,Error) %>% 
              distinct(), aes(sample= Error))+stat_qq() + stat_qq_line()+
xlab("Normalized mean error in kt")+
ylab("Actual mean error in kt")  

g+scheme_basic

ggsave( paste0('Outputs/QQ plot showing normalized error vs actual error in kt for NEI inventory','.png'),width = 10, height = 6 )

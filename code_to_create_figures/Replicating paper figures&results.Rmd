...
tmp_NEI %>% left_join((tmp_NEI_direction), by= c("PlantName","source")) %>%
  na.omit() %>% mutate(source = "NEI (Comprehensive point sources)") %>% dplyr::select(PlantName,Error_Term,Error_percent,Average_Emissions_KT,source) %>% distinct()->plot_data
...
d <- density(na.omit(plot_data$Error_percent),n=1e4)
...
g <- ggplot()+
  ...
  geom_density(data= plot_data, aes(x=Error_percent, y = ..density..,label="Smoothed density based on error distribution"),color="red", size = 1.2)+
  geom_line(data= draw_d, aes(x=Error_percent, y=density),color="blue", linetype = "dashed", size = 1.2)+
  geom_text(data=plot_data, aes(x= max(300)*0.8,y=max(0.01),label= paste0("FWHM - ",FWHM)))+
  geom_text(data=plot_data, aes(x= max(300)*0.77,y=max(0.0096),label= paste0("n - ",length(plot_data$PlantName))))+
...
```
#SI Figure 1: Density plot of percent error for all detected sources in the EGRIDS inventory (Based on 4 years of data)
```{r}
EGRIDS_Data_Mean %>%
  mutate(Error=Value-ValuefromEGRIDSinKT) %>%
  #Now filter out detected sources
  filter(Status=="Detected",
         Year %in% c(2010,2012,2014,2016)) %>%
  #Add selection criteria. Add counts and calculate average emissions
  mutate(Obs=1) %>%
  group_by(PlantName) %>% 
  mutate(obs_count=sum(Obs)) %>% 
...
g <- ggplot()+
  ...
  geom_line(data= draw_d, aes(x=Error_percent, y=density),color="blue", linetype = "dashed", size = 1.2)+
  geom_text(data=plot_data, aes(x= max(300)*0.8,y=max(0.01),label= paste0("FWHM - ",FWHM)))+
  geom_text(data=plot_data, aes(x= max(300)*0.76,y=max(0.0096),label= paste0("n - ",length(plot_data$PlantName))))+
  xlab("Mean percent error")+
  xlim(-150,300)+
  ggtitle("Density plot of percent error for all detected sources in the EGRIDS inventory (Based on 4 years of data)")+
  labs(subtitle = "Bars are the actual density of the error. The red line is a smoothed density curve")+
  ylim(0,0.0125)
g+scheme_basic

ggsave( paste0('Outputs/Density plot of percent error for all detected sources in the EGRIDS inventory ( Last 4 years)','.png'),width = 10, height = 6 )
```

#SI Figure 2: Density plot of percent error for all detected sources in the EGRIDS inventory (Based on 6 years of data)
```{r}
EGRIDS_Data_Mean %>%
  mutate(Error=Value-ValuefromEGRIDSinKT) %>%
  #Now filter out detected sources
